...
def condition_for_model(
    condition: StateCondition, timeout: Optional[timedelta] = None
) -> Condition:
...
class DeviceStateCondition(Condition):
    ...
    def initialize(
        ...
        self._device_value = self._cast_value(
            attrs[self._model.device_id][self._model.attribute]
        )
...
class BooleanStateCondition(Condition):
    ...
    def initialize(
        ...
        for condition_id, state in conditions.items():
            self._subconditions[condition_id] = (
                self._subconditions[condition_id][0],
                state,
            )
        return self.evaluate()
    ...
    def on_condition_event(self, condition: Condition, triggered: bool):
        if condition.identifier in self._subconditions:
            self._subconditions[condition.identifier] = (
                self._subconditions[condition.identifier][0],
                triggered,
            )
