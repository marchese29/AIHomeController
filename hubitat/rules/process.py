...
class Condition(ABC):
    ...
    def duration(self) -> Optional[int]:
        """The duration (in seconds) that this condition must remain true before triggering
        actions."""
        return None
    ...
    def trigger_always(self) -> bool:
        """If true, actions will always be invoked whenever the condition remains true after its
...
class ConditionManager:
    ...
    async def remove_condition(self, condition: "Condition"):
        """Removes a condition from the process manager."""
        await self._rpm.remove_condition(condition)
...
class RuleProcessManager:
    ...
    def __init__(self, he_client: HubitatClient):
        ...
        # Protects the integrity of condition tracking structures
        self._condition_lock = aio.Lock()

    async def add_condition(self, condition: Condition):
        """Adds a new condition to the process manager"""
        async with self._condition_lock:
            await self._add_condition_locked(condition)

    async def _add_condition_locked(self, condition: Condition):
    ...
    async def remove_condition(self, condition: Condition):
        """Removes a condition from the process manager"""
        async with self._condition_lock:
            self._remove_condition_locked(condition)
    ...
    async def _fetch_new_attributes(self, new_device_attrs: dict[int, set[str]]):
        for device_id, attrs in new_device_attrs.items():
            for attr in attrs:
                ...
                self._latest_attributes[device_id][
                    attr
                ] = await self._he_client.get_attribute(device_id, attr)
    ...
    async def _process_condition_change(self, impacted: list[Condition]):
        ...
        for condition in [c for c in touched_conditions if c.action is not None]:
            ...
            if (curr is True and prev is False) or (
                curr is True and condition.trigger_always
            ):
