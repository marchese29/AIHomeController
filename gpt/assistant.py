"""Module for managing the OpenAI Assistant API integration.

This module provides functionality to interact with OpenAI's Assistant API, specifically
designed for home automation control. It handles the creation and management of assistants,
threads, and message processing, including the execution of function calls.

The main class, PromptAssistant, manages the lifecycle of an OpenAI assistant and thread,
handling message processing and tool execution. It supports both creating new assistants
and threads, as well as reusing existing ones based on environment variables.
"""
...
class AIHomeControlAssistant:
            ...
            if self._assistant is not None:
                aio.run(
                    self._client.beta.assistants.update(
                        ...
                        model=self._model,
                        instructions=prompt,
                        tools=[f.get_definition() for f in self._tool_map.values()],
                    )
                )
    ...
    async def handle_user_message(self, message: str) -> str:
        ...
        await self._client.beta.threads.messages.create(
            self._thread.id, role="user", content=message
        )
    ...
    async def _handle_tool_calls(
        ...
        await self._client.beta.threads.runs.submit_tool_outputs(
            run_id=run_id, thread_id=self._thread.id, tool_outputs=outputs
        )

    async def _handle_tool_call(
        self, call: RequiredActionFunctionToolCall
    ) -> JSONObject:
        if call.function.name not in self._tool_map:
            return {
        ...
        return {
            "tool_call_id": call.id,
            "output": await self._tool_map[call.function.name].invoke(
