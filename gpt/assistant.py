...
class PromptAssistant:
    ...
    def __init__(
        ...
        self._model = model
        self._tool_map = {
            f.get_name(): f for f in tools} if tools is not None else {}
        ...
        if self._assistant is None:
            ...
            print(
                f"Main home assistant created with id: '{self._assistant.id}'")
        if self._assistant is None:
            raise RuntimeError(
                "Failed to instantiate assistant for main prompt")

        # Get the existing thread or create a new one
        thread_id = env_var('THREAD_ID', True)
        self._thread: Optional[Thread] = None
        if thread_id is not None:
            self._thread = aio.run(
